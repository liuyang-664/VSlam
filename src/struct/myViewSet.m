% This is the same as Matlab's viewset with the points in camera corrdinate
% for mapping
...
%   imageviewset instead of viewSet.
%   ----------------------------------------------------------------------- 
% 
...
%
%       % Add a view containing new feature points
%       vSet = addView(vSet, i, 'Points', points);
%
%       % Match features against previous frame
%       pairsIdx = matchFeatures(featuresPrev, features);
%
%       % Add a connection with the found matches
...
classdef myViewSet
    ...
    methods
        ...
        %------------------------------------------------------------------
        function this = addView(this, view, points_camera, varargin)
            ...
            % ADDITION
			% Update points in camera cooridinate
			this.PointsCamera{end+1} = points_camera;
        end
